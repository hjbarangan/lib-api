...
const createUser = async (user) => {
  ...
  const result = await pool.query("CALL add_user($1, $2, $3, $4, $5)", [
    first_name,
    last_name,
    username,
    password,
...
const updateUser = async (userId, user) => {
  const { first_name, last_name, username, password, role } = user;
  const result = await pool.query("CALL update_user($1, $2, $3, $4, $5, $6)", [
    first_name,
    last_name,
    username,
    password,
...
const deleteUser = async (userId) => {
  const result = await pool.query("CALL delete_user($1)", [userId]);
  return result.rows[0];
};
